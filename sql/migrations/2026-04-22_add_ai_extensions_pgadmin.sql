-- ============================================================================
-- STRICT PGADMIN MIGRATION - AI EXTENSIONS
-- ============================================================================
-- Execution order:
--   1. Run this file after the core ERP schema is already installed.
--   2. If this migration succeeds, the AI persistence layer is ready.
--   3. If one prerequisite is missing, the migration stops with a clear error.
--
-- Recommended before execution:
--   - backup the database
--   - confirm tables users, customers, products, warehouses exist
-- ============================================================================
    ...
    SELECT ARRAY(
        ...
        WHERE NOT EXISTS (
            ...
            WHERE table_schema = 'public'
              AND table_name = required.required_name
        )
    )
    INTO missing_tables;

    IF array_length(missing_tables, 1) IS NOT NULL THEN
        RAISE EXCEPTION 'Migration AI impossible. Tables manquantes: %', array_to_string(missing_tables, ', ');
    END IF;
END $$;

-- Main additive schema
...
CREATE TABLE IF NOT EXISTS ai_forecasts (
    ...
    CONSTRAINT ai_forecasts_horizon_days_chk CHECK (horizon_days > 0),
    CONSTRAINT ai_forecasts_horizon_dates_chk CHECK (horizon_end >= horizon_start),
...
CREATE TABLE IF NOT EXISTS product_demand_snapshots (
    ...
    CONSTRAINT product_demand_snapshots_quantity_sold_chk CHECK (quantity_sold >= 0),
    CONSTRAINT product_demand_snapshots_sales_amount_chk CHECK (sales_amount >= 0),
    CONSTRAINT product_demand_snapshots_stockout_events_chk CHECK (stockout_events >= 0),
...
CREATE TABLE IF NOT EXISTS campaign_performance (
    ...
    CONSTRAINT campaign_performance_spend_chk CHECK (spend_amount >= 0),
    CONSTRAINT campaign_performance_leads_chk CHECK (leads_count >= 0),
    CONSTRAINT campaign_performance_conversions_chk CHECK (conversions_count >= 0),
    CONSTRAINT campaign_performance_revenue_chk CHECK (revenue_amount >= 0)
);
...
CREATE TABLE IF NOT EXISTS document_chunks (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
    CONSTRAINT document_chunks_chunk_index_chk CHECK (chunk_index >= 0),
...
BEGIN
    ...
    IF array_length(missing_tables, 1) IS NOT NULL THEN
        RAISE EXCEPTION 'Migration AI incomplète. Tables manquantes après création: %', array_to_string(missing_tables, ', ');
    END IF;
END $$;

COMMIT;

-- Optional manual checks after execution:
--
-- SELECT table_name
-- FROM information_schema.tables
-- WHERE table_schema = 'public'
...
-- WHERE table_schema = 'public'
--   AND table_name IN ('ai_agent_runs', 'ai_alerts', 'ai_recommendations')
